...

{
    /* delta com*/
    _delta_com.setZero();
    _mpc_solver = std::make_shared<MpcSolver>(_opt.h, _opt.Ts, _opt.T, _dt, _opt.Q, _opt.R);
}
...
//    double time = _alpha * fabs( duration_step );
    ...
    _zmp_window.head(size_current) << _zmp_val;
                          ...
                          double offset)
{
    ...
    computePreviewWindow(q_sns,q_max, q_min, zmp_val_current, zmp_val_next, duration_preview_window, duration_step, middle_zmp, offset);
