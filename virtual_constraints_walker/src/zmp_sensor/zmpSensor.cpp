...
void zmpSensor::update(Eigen::Affine3d w_T_Lfoot, Eigen::Affine3d w_T_Rfoot)
{
    ...
    {
        _ZMP_L = computeFootZMP(_FT_foot_L, _ankle_T_foot_L.translation());
    }
    
    if (_FT_foot_R(2) >= _Fz_min)
    {
        _ZMP_R = computeFootZMP(_FT_foot_R, _ankle_T_foot_R.translation());
    }
    
    std::cout << "w_T_Lfoot: " << _w_T_Lfoot.translation().transpose() << std::endl;
    std::cout << "w_T_Rfoot: " << _w_T_Rfoot.translation().transpose()  << std::endl;
    std::cout << "ZMP_L: " <<_ZMP_L.transpose() << std::endl;
    std::cout << "ZMP_R: " <<_ZMP_R.transpose()  << std::endl;
    
    /* get ZMP w.r.t. world */
    Eigen::Vector3d w_T_ZMP_L = _ZMP_L + _w_T_Lfoot.translation();
    Eigen::Vector3d w_T_ZMP_R = _ZMP_R + _w_T_Rfoot.translation();
    ...
    _logger->add("ZMP_L", _ZMP_L);
    _logger->add("ZMP_R", _ZMP_R);
